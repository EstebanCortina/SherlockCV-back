 ...
 */
export async function indexAsync (req: Request, res: Response): Promise<void> {
 ...
 */
export async function showAsync (req: Request, res: Response): Promise<any> {

    const jobPosition = await (
        controllerModel.getJobPositionById(req.params.id, req.userId)
    );

    if(!validateJobPosition(jobPosition)){
 ...
 */
export async function createAsync (req: Request, res: Response): Promise<void> {
 ...
 */
export async function updateAsync (req: Request, res: Response): Promise<any> {

    const jobPositionToUpdate = await (
        controllerModel.getJobPositionById(req.params.id, req.userId)
    );

    if(!validateJobPosition(jobPositionToUpdate)){
        return res.status(404).send(badRequest(
            400, "Job Position Not found"
        ))
    }
 ...
 * @returns {Promise<any>} Sends a response with a status of 200 and the updated job position data if the update is successful, otherwise sends a 404 response.
 */
export async function toggleIsOpenAsync (req: Request, res: Response): Promise<any> {
 ...
 */
export async function deleteSoftAsync (req: Request, res: Response): Promise<any> {

    const jobPositionToDelete = await (
        controllerModel.getJobPositionById(req.params.id, req.userId)
    );

    if(!validateJobPosition(jobPositionToDelete)){
        return res.status(404).send(badRequest(
            400, "Job Position Not found"
        ))
    }
 ...
 */
function validateJobPosition(jobPosition: Object): boolean{
